...
public class SLockSearchActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_slock_search);
        ButterKnife.bind(this);
        titleBarTitle.setText("锁库单搜索");
    ...
    public void onViewClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.rl_order_end_date:
                DatePickerDialog datePicker = new DatePickerDialog(SLockSearchActivity.this,
            ...
            case R.id.rl_order_start_date:
                DatePickerDialog datePickerend = new DatePickerDialog(SLockSearchActivity.this,
                        new DatePickerDialog.OnDateSetListener() {
    ...
    private String getFormJson() {
        ...
        if (!textEndDate.getText().toString().isEmpty()) {
            String endDate = "s_lock_hsdate_1" + ":" + new Date(textEndDate.getText().toString()).getTime() + ",";
            upJson += endDate;
        }
        if (!textStartDate.getText().toString().isEmpty()) {
            String endDate = "s_lock_hedate_1" + ":" + new Date(textEndDate.getText().toString()).getTime() + ",";
            upJson += endDate;
        }
        if (!sLockH02.getText().toString().isEmpty()) {
            String filterB = "s_lock_h02" + ":\"" + sLockH02.getText().toString() + "\",";
            upJson += filterB;
        }
        if (!sLockH01.getText().toString().isEmpty()) {
            String filterB = "s_lock_h01" + ":\"" + sLockH01.getText().toString() + "\",";
            upJson += filterB;
        }
        if (!sLockE05.getText().toString().isEmpty()) {
            String filterB = "s_lock_e06" + ":\"" + sLockE05.getText().toString() + "\",";
            upJson += filterB;
        }

        if (!sLockE07.getText().toString().isEmpty()) {
            String filterB = "s_lock_e07" + ":\"" + sLockE07.getText().toString() + "\",";
            upJson += filterB;
        }
        if (!sLockE08.getText().toString().isEmpty()) {
            String filterB = "s_lock_e08" + ":\"" + sLockE08.getText().toString() + "\",";
            upJson += filterB;
        }
        return upJson;
    }
    ...
    //  url
    private String search_lock_order_url = Const.W_HOST + "/api/stockData/searchLockSkList";
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case GET_DATA_IS_NULL:
                    ToastUtil.showToast(SLockSearchActivity.this, "您输入的条件没有匹配的订单");
